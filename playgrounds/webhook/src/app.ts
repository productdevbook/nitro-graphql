...
const wsUrl = `${wsProtocol}//${window.location.host}/api/graphql/ws`
...
const wsClient = createClient({
  ...
  on: {
    ...
    closed: () => console.log('[GraphQL WS] Disconnected'),
    error: (error) => console.error('[GraphQL WS] Error:', error),
  },
})

console.log('[GraphQL WS] Client created with URL:', wsUrl)
...
export function setupApp(element: HTMLButtonElement) {
  ...
  createButton('Subscribe to User Created', '#8b5cf6', () => {
    if (subscriptionUnsubscribe) {
      subscriptionUnsubscribe()
      subscriptionUnsubscribe = null
      results.innerHTML = '<div style="color: #ef4444; font-weight: 600;">✗ Unsubscribed from user created events</div>'
      return
    }

    results.innerHTML = '<div style="color: #8b5cf6; font-weight: 600;">⚡ Subscribed! Waiting for new users...</div>'

    subscriptionUnsubscribe = wsClient.subscribe(
      {
        query: `
          subscription {
            userCreated {
      ...
      },
      {
        next: (result: any) => {
          if (result.data) {
            ...
            results.innerHTML = `
              <div style="color: #8b5cf6; font-weight: 600; margin-bottom: 12px;">⚡ New User Created (${timestamp})</div>
              <pre style="background: #2d2d2d; color: #e0e0e0; padding: 16px; border-radius: 6px; font-size: 13px; border: 1px solid #3d3d3d; margin-bottom: 16px;">${JSON.stringify(result.data.userCreated, null, 2)}</pre>
              ${currentContent.includes('Subscribed!') ? '' : currentContent}
            `
          }
        },
        error: (error: any) => {
          ...
          results.innerHTML = `
            <div style="color: #ff6b6b; font-weight: 600; margin-bottom: 12px;">✗ Subscription Error</div>
            <pre style="color: #ff6b6b; margin: 0;">${error.message || error}</pre>
          `
          subscriptionUnsubscribe = null
        },
        complete: () => {
