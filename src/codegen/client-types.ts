...
export async function generateMainClientTypes(
  ...
  if (nitro.options.graphql?.subscriptions?.enabled) {
    const wsEndpoint = nitro.options.runtimeConfig?.graphql?.endpoint?.ws || '/api/graphql/ws'
    generateWebSocketClient(nitro, SERVICE_DEFAULT, wsEndpoint)
  }
...
/**
 * Generate WebSocket client wrapper for GraphQL subscriptions
 */
